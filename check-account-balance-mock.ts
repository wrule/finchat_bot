...
async function checkAccountBalanceMock() {
  ...
  console.log('💰 [MOCK] 虚拟盘账户资产查询');
  console.log('='.repeat(80));
  console.log('');

  try {
    // 1. 获取账户资产
    console.log('📊 正在获取虚拟账户资产...');
    const assets = await client.getContractAccountAssets();
    ...
    if (positions.length === 0) {
      ...
      positions.forEach((pos: any, index: number) => {
        ...
        console.log(`  开仓均价:     ${formatNumber(pos.entryPrice, 2)} USDT`);
    ...
    const risk = await client.getAccountRiskForAI('cmt_btcusdt');
    ...
    console.log('');
    console.log('='.repeat(80));
    console.log('📊 [MOCK] 虚拟盘统计');
    console.log('='.repeat(80));

    const stats = await client.getMockStatistics();
    console.log(`💰 初始资金:       ${formatNumber(stats.initialBalance, 2)} USDT`);
    console.log(`💰 当前资金:       ${formatNumber(stats.currentBalance, 2)} USDT`);
    console.log(`📈 总盈亏:         ${stats.totalPnl >= 0 ? '+' : ''}${formatNumber(stats.totalPnl, 2)} USDT (${stats.pnlPercent >= 0 ? '+' : ''}${formatNumber(stats.pnlPercent, 2)}%)`);
    console.log(`📊 交易次数:       ${stats.tradesCount}`);
    console.log(`🎯 胜率:           ${formatNumber(stats.winRate, 1)}%`);
